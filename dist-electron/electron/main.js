...
app.whenReady().then(async () => {
    try {
        ...
        if (process.env.VITE_DEV_SERVER_URL) {
            ...
            if (fs.existsSync(htmlPath)) {
                ...
                if (!loaded) {
                    ...
                    // Create error page
                    win.loadURL(`data:text/html,<h1>Error: HTML file not found</h1><p>Checked paths:</p><ul><li>${htmlPath}</li>${altPaths
                        .map((p) => `<li>${p}</li>`)
                        .join("")}</ul>`);
                }
            }
        ...
        errorWin.loadURL(`data:text/html,<h1>Error: ${error.message}</h1><pre>${error.stack}</pre>`);
    }
});
