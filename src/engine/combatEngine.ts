...
export function computeRoundStep(
  ...
  const { roundSpinResults, disabledItems, allPlayers, getPerRoundEffects } = ctx;
  ...
  const p1HasSpellFlux = (p1.character?.powers || []).some(
    (pw: any) => !pw?.isLost && (typeof pw === 'string' ? pw : pw.name)?.toLowerCase().startsWith('spell flux'),
  );
    ...
    for (const ce of fx.combatEffects) {
      ...
      if (!handlerName) {
        ...
        if (
          ...
          if (hasSpellFlux && !spellFluxUsed && timing === 'during_combat' && ce.source?.type === 'power') {
            spellFluxUsed = true;
      ...
      const isSpellFluxTarget = hasSpellFlux && !spellFluxUsed && timing === 'during_combat' && ce.source?.type === 'power';
  ...
  if (roundIndex < 5) {
    const addSilverRangerCarryOver = (winnerSide: 'player1' | 'player2') => {
      ...
      if (!spinResult?.isSuccess) return;
      const alreadyAdded = carryOverToNext.some((co) => co.player === winnerSide && co.source === 'Ranger-Silver');
      ...
      carryOverToNext.push({ player: winnerSide, source: 'Ranger-Silver', stat: nextStatKey, value: currentStatValue, description: `Silver Ranger: gấp đôi ${nextStatKey.toUpperCase()} round kế (+${currentStatValue})` });
      events.push({ player: winnerSide, source: 'Ranger-Silver', description: `Silver Ranger: +${currentStatValue} ${nextStatKey.toUpperCase()} carry sang round kế (gấp đôi)`, type: 'carry_over' });
    };
    const p1EffsSR = getPerRoundEffects(p1.character, p1.no);
    const p2EffsSR = getPerRoundEffects(p2.character, p2.no);
    if (winner === 'player1' && p1EffsSR.onWin.includes('Ranger-Silver')) addSilverRangerCarryOver('player1');
    if (winner === 'player2' && p2EffsSR.onWin.includes('Ranger-Silver')) addSilverRangerCarryOver('player2');
  }
  ...
  const roundResult: RoundResult = { stat: key, statLabel: label, player1Value: p1Val, player2Value: p2Val, winner };
  const log: RoundLog = {
    roundIndex,
    statLabel: label,
    statKey: key,
