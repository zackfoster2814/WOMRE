...
export function registerAllQuirkEffects() {
  ...
  defineEffect('quirk', 'Lazy')
    ...
    .effect({ type: 'stat_modifier', stat: 'iq', value: 2, timing: 'after_combat', target: 'self' })
    .register();
  ...
  defineEffect('quirk', 'Bloodthirsty')
    ...
    .effect({ type: 'lose_points_on_lose', points: -999, timing: 'on_round_lose', target: 'self' })
    .register();
  ...
  defineEffect('quirk', 'Generous')
    ...
    .weight(2.38)
    .effect({ type: 'custom', timing: 'after_combat_win', target: 'self', customHandler: 'generous_give_reward' })
    .effect({ type: 'custom', timing: 'after_combat_lose', target: 'self', customHandler: 'generous_receive_bonus' })
    .register();
  ...
  defineEffect('quirk', 'Cheater')
    ...
    .weight(2.38)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'immediate', target: 'self', customHandler: 'cheater_multi_lover_check' })
    .effect({ type: 'stat_modifier', stat: 'biq', value: -2, timing: 'immediate', target: 'lover' })
    .effect({ type: 'custom', timing: 'on_death', target: 'self', customHandler: 'cheater_death_buff_lovers' })
    .register();
  ...
  defineEffect('quirk', 'Deaf')
    ...
    .weight(2.38)
    .effect({ type: 'immunity', immuneTo: ['instrument_effects'], timing: 'immediate', target: 'self' })
    .register();
  ...
  defineEffect('quirk', 'Patient')
    ...
    .weight(2.38)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'patient_no_power_wheel' })
    .effect({ type: 'custom', timing: 'after_combat_win', target: 'self', customHandler: 'patient_max_power_wheel' })
    .register();
  ...
  defineEffect('quirk', 'Impatient')
    ...
    .weight(2.38)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'impatient_instant_rewards' })
    .effect({ type: 'immunity', immuneTo: ['pvp_reward'], timing: 'immediate', target: 'self' })
    .register();

  // One Trick Pony
  defineEffect('quirk', 'One Trick Pony')
    .description('Trong combat: Quay vòng 6 chỉ số để chọn. Thắng chỉ số đó = 3 điểm; thắng các chỉ số còn lại = 0 điểm.')
    .weight(2.38)
    .effect({ type: 'custom', timing: 'during_combat', target: 'self', customHandler: 'one_trick_pony_stat_selection' })
    .register();
  ...
  defineEffect('quirk', 'Cruelty')
    ...
    .weight(2.38)
    .effect({ type: 'custom', timing: 'on_round_tie', target: 'self', customHandler: 'cruelty_tie_coinflip' })
    .register();
}
...
registerImmediateHandler(
  ...
  (ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    ...
    if (rewardsGiven === 0) {
      return { skipDefault: true, description: 'Generous: chưa tặng PvP Reward nào → không nhận bonus' };
    }
...
registerImmediateHandler(
  ...
  (ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    ...
    if (loverList.length === 0) {
      return { skipDefault: true, description: 'Cheater: không có Lover → không có death buff' };
    }
    return {
      skipDefault: true,
...
registerImmediateHandler(
  ...
  (ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    ...
    if (/nhận\s+power|tặng/i.test(quirkName)) {
      return { skipDefault: true, description: `${loverChar.name} đã tặng Power` };
    }
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    return {
      selfStatMods: STAT_NAMES.map(stat => ({ stat, value: ctx.self.stats[stat] * 7 })),
