...
registerImmediateHandler(
  ...
  (ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    ...
    if (hasQuas && hasWex && hasExort) {
      return { skipDefault: false, description: 'Invoker unlocked (has Quas+Wex+Exort)' };
    }
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: 'Weapon stats enhanced (passive)' };
  },
...
registerImmediateHandler(
  ...
  (ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    ...
    if (quirkCount > 0) {
      return { skipDefault: false, description: `Will receive ${quirkCount} Power(s) from Quirkful` };
    }
    return { skipDefault: true };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: 'Weapons removed for Fist Fighting' };
  },
  'Remove weapons',
);
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: '+1 all stats from Round 32 (passive)' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: 'Luck Manipulation bonus (passive)' };
  },
...
registerImmediateHandler(
  'lone_wolf_check',
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: 'Lone Wolf power check (after combat)' };
  },
...
registerImmediateHandler(
  ...
  (ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    ...
    return { skipDefault: true, description: '+1 all stats vs Vampire (combat)' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: '+2 Strength converts to highest stat on loss' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: 'Debuffs are swapped between players' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: 'Disable opponent "during combat" power' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: '-1 highest stat per gear (max 5)' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: '+1 to 2 random stats if opponent has fewer powers' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: '+1 to opponent highest stat on win' };
  },
...
registerImmediateHandler(
  ...
  (_ctx: ImmediateHandlerContext): ImmediateHandlerResult => {
    return { skipDefault: true, description: '+1 MA if won MA round and combat (stacks)' };
  },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    if (activations > 0) {
      return { selfStatMods: [{ stat: 'iq', value: activations }], description: `+${activations} IQ (power activations)` };
    }
    return { skipDefault: true };
  },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    if (ctx.self.roundsLost >= 2) {
      return { opponentPoints: 0, description: 'Opponent gets no point on next round win' };
    }
    return { skipDefault: true };
  },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    return { description: `Luck Manipulation: Từ vòng 64 → bonus stats sẽ áp dụng` };
  },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    const hasSoundPower = powers.some(p => SOUND_POWERS.some(sp => p.includes(sp)));
    if (hasSoundPower) {
      return {
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    return { skipDefault: true, description: 'Bucking Bronco: Không thắng round MA → không nhận bonus' };
  },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    if (highestStat === 'strength') {
      return { skipDefault: true, description: 'Odin Blessing: Stat cao nhất là Strength → +2 STR như bình thường' };
    }
    return {
      selfStatMods: [
        { stat: 'strength', value: -2 },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    return { description: `Fancy Feet: Vô hiệu Rune/Runeword của đối thủ (${runeDesc}) trước combat` };
  },
...
registerCombatHandler(
  ...
  (ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    const breathPowers = ctx.self.powers.filter(p => p.toLowerCase().includes('breath')).length;
...
registerCombatHandler(
  ...
  (_ctx: CombatHandlerContext): CombatHandlerResult => {
    ...
    if (Math.random() < 0.10) {
      return { autoWin: true, description: 'LẬT KÈO! (Super Lucky – 10% lần 2)' };
    }
    return { skipDefault: true, description: 'Super Lucky: không lật kèo được (cả 15% lẫn 10% đều trượt)' };
  },
...
export function registerAllPowerEffects() {
  ...
  defineEffect('power', 'U=ma2')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'speed', value: 3, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'uma2_str_lose' })
    .effect({ type: 'stat_modifier', stat: 'durability', value: 4, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'uma2_spd_lose' })
    .register();

  defineEffect('power', 'Memory Freeze')
    .description('Sau combat thua: Đối thủ nhận Quirk "Brainrot".')
    .weight(0.78)
    .effect({ type: 'grant_quirk', grantType: 'quirk', grantName: 'Brainrot', grantCount: 1, timing: 'after_combat_lose', target: 'opponent' })
    .register();

  defineEffect('power', 'Metamagic')
    .description('Trong Combat: +1 Điểm ở Round BIQ nếu thắng.')
    .weight(0.78)
    .effect({ type: 'extra_point_on_win', points: 1, timing: 'during_combat', target: 'self', customHandler: 'metamagic_biq_round' })
    .register();
  ...
  defineEffect('power', 'Critical Strike')
    ...
    .weight(0.78)
    .effect({ type: 'extra_point_on_win', points: 1, timing: 'on_round_win', target: 'self', conditions: [{ type: 'probability', chance: 20 }] })
    .register();

  defineEffect('power', 'Evasion')
    .description('Trong Combat: 20% nhận 1 điểm mỗi round thua.')
    .weight(0.78)
    .effect({ type: 'combat_points', points: 1, timing: 'on_round_lose', target: 'self', conditions: [{ type: 'probability', chance: 20 }] })
    .register();
  ...
  defineEffect('power', 'Healing Factor')
    ...
    .effect({ type: 'stat_modifier', stat: 'durability', value: 1, timing: 'after_combat', target: 'self', customHandler: 'healing_factor_per_2_lost' })
    .register();
  ...
  defineEffect('power', 'Lone Wolf')
    ...
    .weight(0.78)
    .addStat('speed', 3)
    .effect({ type: 'custom', timing: 'after_combat', target: 'self', customHandler: 'lone_wolf_check' })
    .register();

  defineEffect('power', 'AIDS')
    .description('Sau Combat: -2 Dura. Lây sang Lover. Không thể xóa bỏ.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'durability', value: -2, timing: 'after_combat', target: 'self' })
    .effect({ type: 'custom', timing: 'immediate', target: 'lover', customHandler: 'aids_spread_to_lover' })
    .effect({ type: 'immunity', immuneTo: ['remove_aids'], timing: 'immediate', target: 'self' })
    .register();
  ...
  defineEffect('power', 'Magma Strike')
    ...
    .effect({ type: 'stat_modifier', stat: 'durability', value: -1, timing: 'after_combat', target: 'opponent' })
    .register();
  ...
  defineEffect('power', 'Storm Calling')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'biq', value: 2, timing: 'before_combat', target: 'self' })
    .register();
  ...
  defineEffect('power', 'Spear of Fire')
    ...
    .weight(0.78)
    .addStat('ma', 2)
    .effect({ type: 'combat_points', points: 1, timing: 'before_combat', target: 'self', customHandler: 'spear_of_fire_2_rune_check' })
    .register();
  ...
  defineEffect('power', 'Divine Lightning')
    ...
    .addStat('speed', 1)
    .addStat('ma', 1)
    .effect({ type: 'stat_modifier', stat: 'strength', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon'] }] })
    .effect({ type: 'stat_modifier', stat: 'speed', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon'] }] })
    .effect({ type: 'stat_modifier', stat: 'ma', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon'] }] })
    .register();

  defineEffect('power', 'Fist Fighting')
    .description('Mất vũ khí. +4 MA.')
    .weight(0.78)
    .effect({ type: 'remove_gear', timing: 'immediate', target: 'self', customHandler: 'fist_fighting_remove_weapon' })
    .addStat('ma', 4)
    .register();
  ...
  defineEffect('power', 'Rampage')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'after_combat_win', target: 'self', conditions: [{ type: 'probability', chance: 36 }] })
    .register();

  defineEffect('power', 'Bloodlust')
    .description('Sau combat thắng: -1 IQ/MA, +1 Str/Spd, +2 Dura.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: -1, timing: 'after_combat_win', target: 'self' })
    .effect({ type: 'stat_modifier', stat: 'ma', value: -1, timing: 'after_combat_win', target: 'self' })
    .effect({ type: 'stat_modifier', stat: 'strength', value: 1, timing: 'after_combat_win', target: 'self' })
    .effect({ type: 'stat_modifier', stat: 'speed', value: 1, timing: 'after_combat_win', target: 'self' })
    .effect({ type: 'stat_modifier', stat: 'durability', value: 2, timing: 'after_combat_win', target: 'self' })
    .register();

  defineEffect('power', 'Power Absorption')
    .description('Sau combat thắng: hấp thụ 1 Power ngẫu nhiên của đối thủ.')
    .weight(0.78)
    .effect({ type: 'steal_power', grantCount: 1, timing: 'after_combat_win', target: 'opponent' })
    .register();

  defineEffect('power', 'Power Negation')
    .description('Trước Combat: Vô hiệu hóa 1 Power ngẫu nhiên của đối thủ.')
    .weight(0.78)
    .effect({ type: 'power_disable', timing: 'before_combat', target: 'opponent', grantCount: 1 })
    .register();

  defineEffect('power', 'Mind Control')
    .description('Trước Combat: +1 all stats vs đối thủ có Base IQ ≤5.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'before_combat', target: 'self', conditions: [{ type: 'stat_compare', stat: 'iq', compareWith: 'opponent', operator: '<=' }], customHandler: 'mind_control_iq_check' })
    .register();

  defineEffect('power', 'Blood Manipulation')
    .description('Trước Combat: +3 Strength vs tất cả trừ Skeleton và Spirit.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'strength', value: 3, timing: 'before_combat', target: 'self', conditions: [{ type: 'race_match', excludeRaces: ['Skeleton', 'Spirit'] }] })
    .register();
  ...
  defineEffect('power', 'Armor Piercing')
    ...
    .weight(0.78)
    .effect({ type: 'extra_point_on_win', points: 1, timing: 'during_combat', target: 'self', customHandler: 'armor_piercing_dura_round' })
    .register();

  defineEffect('power', 'Divine Smite')
    .description('Trong Combat: +1 Điểm ở Round MA nếu thắng.')
    .weight(0.78)
    .effect({ type: 'extra_point_on_win', points: 1, timing: 'during_combat', target: 'self', customHandler: 'divine_smite_ma_round' })
    .register();
  ...
  defineEffect('power', 'Quas')
    ...
    .weight(0.78)
    .addStat('strength', 1)
    .effect({ type: 'grant_archetype', grantName: 'Invoker', grantCount: 1, timing: 'immediate', target: 'self', triggerOnce: true, customHandler: 'quas_wex_exort_check' })
    .register();
  ...
  defineEffect('power', 'Bloody Strike')
    ...
    .weight(0.78)
    .addAllStats(-1)
    .effect({ type: 'stat_modifier', stat: 'random', value: 1, timing: 'after_combat', target: 'self', customHandler: 'bloody_strike_per_round_won' })
    .register();

  defineEffect('power', 'Accelerating Sorcery')
    .description('Trong Combat: +1 IQ mỗi khi kích hoạt Power "Trong combat".')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: 1, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'accelerating_sorcery_count' })
    .register();

  defineEffect('power', 'Homeguard')
    .description('Trong Combat: Buff +3 Speed nếu không thua round Strength.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'speed', value: 3, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'homeguard_no_str_lose' })
    .register();

  defineEffect('power', 'Bash')
    .description('Trong combat: Sau mỗi round thắng, 35% đối thủ -3 stat round kế.')
    .weight(0.78)
    .effect({ type: 'debuff', stat: 'random', value: -3, timing: 'on_round_win', target: 'opponent', conditions: [{ type: 'probability', chance: 35 }] })
    .register();

  defineEffect('power', 'Gate to Heaven')
    .description('Khi ở nhánh thắng từ Vòng 32: +1 all stats.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'immediate', target: 'self', customHandler: 'gate_to_heaven_round_check' })
    .register();

  defineEffect('power', 'Weapon Enhancing')
    .description('+1 vào mỗi buff từ vũ khí, -1 vào mỗi debuff từ vũ khí.')
    .weight(0.78)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'weapon_enhancing_modify' })
    .register();
  ...
  defineEffect('power', 'Fancy Feet')
    ...
    .weight(0.78)
    .addStat('durability', -1)
    .effect({ type: 'weapon_disable', timing: 'before_combat', target: 'opponent' })
    .effect({ type: 'custom', timing: 'before_combat', target: 'opponent', customHandler: 'fancy_feet_disable_rune' })
    .register();

  defineEffect('power', 'Artist')
    .description('Khi xuống nhánh thua: Stat lẻ của bạn nhận +1. (Dựa theo Base Stats)')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'odd', value: 1, timing: 'on_loser_bracket', target: 'self' })
    .register();

  defineEffect('power', 'Writer')
    .description('Khi xuống nhánh thua: Stat chẵn của bạn nhận +1. (Dựa theo Base Stats)')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'even', value: 1, timing: 'on_loser_bracket', target: 'self' })
    .register();

  defineEffect('power', 'Red Shift/LP1211-M')
    .description('Trong combat: Nếu thắng ít nhất 1/3 round đầu, Buff: +1 IQ, +1 BIQ, +2 MA.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: 1, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'red_shift_check' })
    .effect({ type: 'stat_modifier', stat: 'biq', value: 1, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'red_shift_check' })
    .effect({ type: 'stat_modifier', stat: 'ma', value: 2, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'red_shift_check' })
    .register();
  ...
  defineEffect('power', 'Shooting for Victory')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: 1, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'shooting_for_victory_check' })
    .effect({ type: 'stat_modifier', stat: 'biq', value: 1, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'shooting_for_victory_check' })
    .effect({ type: 'stat_modifier', stat: 'ma', value: 2, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'shooting_for_victory_check' })
    .register();

  defineEffect('power', "Let's Pump Some Iron!")
    .description('Trong combat: Nếu thắng đúng 1/3 round đầu, Buff: +2 IQ, +2 BIQ, +2 MA.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: 2, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'pump_iron_check' })
    .effect({ type: 'stat_modifier', stat: 'biq', value: 2, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'pump_iron_check' })
    .effect({ type: 'stat_modifier', stat: 'ma', value: 2, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'pump_iron_check' })
    .register();

  defineEffect('power', "Hunter's Rewards")
    .description('Sau Combat: Thắng PvP nhận 2 phần thưởng thay vì 1.')
    .weight(0.78)
    .effect({ type: 'double_reward', timing: 'after_combat_win', target: 'self' })
    .register();

  defineEffect('power', 'Quirkful')
    .description('Nhận thêm 1 Power với mỗi Quirk bạn có khi quay ra Power này.')
    .weight(0.78)
    .effect({ type: 'grant_power', grantType: 'power', grantName: 'random', timing: 'immediate', target: 'self', triggerOnce: true, customHandler: 'quirkful_grant_per_quirk' })
    .register();

  defineEffect('power', 'Quirkless')
    .description('Mất hết tất cả Quirk.')
    .weight(0.78)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'quirkless_remove_all_quirks' })
    .register();

  defineEffect('power', 'Sybaurafarming')
    .description('Base Stat >5 thành 5. Mỗi stat bị đổi nhận 1 Quirk.')
    .weight(0.78)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', triggerOnce: true, customHandler: 'sybaurafarming_effect' })
    .register();
  ...
  defineEffect('power', 'EscAPADe')
    ...
    .weight(0.78)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'escapade_convert_iq_to_str' })
    .register();

  defineEffect('power', 'Spirit Link')
    .description('Nhận +1 vào 1 Stat ngẫu nhiên với mỗi 1 người bạn loại.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'random', value: 1, timing: 'after_combat_win', target: 'self', stackable: true })
    .register();

  defineEffect('power', 'The Sand of Time')
    .description('Trong Combat: Lần đầu thua round, 40% +1 điểm bạn và -1 điểm đối thủ.')
    .weight(0.78)
    .effect({ type: 'combat_points', points: 1, timing: 'on_round_lose', target: 'self', triggerOnce: true, conditions: [{ type: 'probability', chance: 40 }] })
    .effect({ type: 'combat_points', points: -1, timing: 'on_round_lose', target: 'opponent', triggerOnce: true, conditions: [{ type: 'probability', chance: 40 }] })
    .register();
  ...
  defineEffect('power', 'Memory Alter')
    ...
    .weight(0.78)
    .effect({ type: 'power_disable', timing: 'before_combat', target: 'opponent', grantCount: 1, customHandler: 'memory_alter_disable_in_combat_power' })
    .register();
  ...
  defineEffect('power', 'Gaze of the Abyss')
    ...
    .weight(0.78)
    .effect({ type: 'extra_point_on_win', points: 5, timing: 'during_combat', target: 'self', customHandler: 'gaze_of_abyss_5_loses' })
    .register();
  ...
  defineEffect('power', 'Master of War')
    ...
    .weight(0.78)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'master_of_war_unlock_all_weapons' })
    .register();
  ...
  defineEffect('power', 'Borrowed Time')
    ...
    .weight(0.78)
    .effect({ type: 'custom', timing: 'during_combat', target: 'self', triggerOnce: true, customHandler: 'borrowed_time_2_loses' })
    .register();

  defineEffect('power', 'Guidance')
    .description('Trước Combat: +1 vào 2 Stat ngẫu nhiên nếu đối thủ có ít power hơn.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'random', value: 1, timing: 'before_combat', target: 'self', customHandler: 'guidance_fewer_powers_check' })
    .register();

  defineEffect('power', "Hunter's Mark")
    .description('Trước Combat: Chọn 1 Round ngẫu nhiên, thắng round đó +1 điểm.')
    .weight(0.78)
    .effect({ type: 'extra_point_on_win', points: 1, timing: 'during_combat', target: 'self', customHandler: 'hunters_mark_random_round' })
    .register();
  ...
  defineEffect('power', 'Cursed')
    ...
    .weight(0.78)
    .effect({ type: 'stat_respin', stat: 'highest', timing: 'immediate', target: 'self', triggerOnce: true })
    .register();
  ...
  defineEffect('power', 'Enhanced Hearing')
    ...
    .weight(0.78)
    .addStat('ma', 2)
    .effect({ type: 'stat_modifier', stat: 'all', value: -1, timing: 'before_combat', target: 'self', customHandler: 'enhanced_hearing_instrument_check' })
    .effect({ type: 'stat_modifier', stat: 'all', value: -2, timing: 'before_combat', target: 'self', customHandler: 'enhanced_hearing_sound_power_check' })
    .register();
  ...
  defineEffect('power', 'Garlic Breath')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'during_combat', target: 'self', customHandler: 'garlic_breath_check' })
    .register();
  ...
  defineEffect('power', 'Anti-Magic Barrier')
    ...
    .weight(0.78)
    .addStat('durability', -1)
    .effect({ type: 'disable_powers', count: 2, timing: 'before_combat', target: 'opponent' })
    .register();
  ...
  defineEffect('power', 'Sacred Fire')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Vampire', 'Demon'] }] })
    .register();
  ...
  defineEffect('power', 'Fair Duel')
    ...
    .weight(0.78)
    .effect({ type: 'immunity', immuneTo: ['debuff'], timing: 'before_combat', target: 'both' })
    .register();
  ...
  defineEffect('power', 'Uno Reverse Card')
    ...
    .weight(0.78)
    .effect({ type: 'custom', timing: 'before_combat', target: 'self', customHandler: 'uno_reverse_card_swap_debuffs' })
    .register();
  ...
  defineEffect('power', 'Chaos Enchantment')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'strength', value: 1, timing: 'after_combat', target: 'self', conditions: [{ type: 'bracket', bracket: 'loser' }] })
    .effect({ type: 'stat_modifier', stat: 'ma', value: 1, timing: 'after_combat', target: 'self', conditions: [{ type: 'bracket', bracket: 'loser' }] })
    .register();
  ...
  defineEffect('power', 'Powerful Strike')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'ma', value: 2, timing: 'immediate', target: 'self', conditions: [{ type: 'has_item', itemType: 'weapon' }] })
    .register();
  ...
  defineEffect('power', 'Odin Blessing')
    ...
    .weight(0.78)
    .addStat('strength', 2)
    .effect({ type: 'custom', timing: 'after_combat_lose', target: 'self', customHandler: 'odin_blessing_convert_to_highest' })
    .register();

  defineEffect('power', 'Misty Step Ahead')
    .description('[PVE ONLY] Trong combat: Tổ đội khởi đầu với 1 điểm.')
    .weight(0.78)
    .effect({ type: 'combat_points', points: 1, timing: 'before_combat', target: 'team', conditions: [{ type: 'always' }] })
    .register();
  ...
  defineEffect('power', 'Luck Manipulation')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'before_combat', target: 'self', conditions: [{ type: 'probability', chance: 15 }], customHandler: 'luck_manipulation_round_64_check' })
    .effect({ type: 'stat_modifier', stat: 'all', value: 2, timing: 'before_combat', target: 'self', conditions: [{ type: 'probability', chance: 5 }], customHandler: 'luck_manipulation_round_64_check' })
    .register();
  ...
  defineEffect('power', 'Bucking Bronco')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'ma', value: 1, timing: 'after_combat_win', target: 'self', stackable: true, customHandler: 'bucking_bronco_ma_round_win' })
    .register();

  defineEffect('power', 'Detect Thoughts')
    .description('Trước Combat: Base IQ cao hơn đối thủ, BIQ trong trận +2.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'biq', value: 2, timing: 'before_combat', target: 'self', conditions: [{ type: 'stat_compare', stat: 'iq', compareWith: 'opponent', operator: '>', useBaseStats: true }] })
    .register();
  ...
  defineEffect('power', 'Analysis Sins')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon', 'Vampire', 'Spirit', 'Orc', 'Skeleton', 'Goblin'] }] })
    .effect({ type: 'stat_modifier', stat: 'strength', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon', 'Vampire', 'Spirit', 'Orc', 'Skeleton', 'Goblin'] }] })
    .register();

  defineEffect('power', 'Cleaning Sins')
    .description('Buff: vs Demon/Vampire/Spirit/Orc/Skeleton/Goblin, +1 BIQ và +1 Dura.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'biq', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon', 'Vampire', 'Spirit', 'Orc', 'Skeleton', 'Goblin'] }] })
    .effect({ type: 'stat_modifier', stat: 'durability', value: 1, timing: 'during_combat', target: 'self', conditions: [{ type: 'race_match', races: ['Demon', 'Vampire', 'Spirit', 'Orc', 'Skeleton', 'Goblin'] }] })
    .register();
  ...
  defineEffect('power', 'Groundwork')
    ...
    .weight(0.78)
    .effect({ type: 'custom', timing: 'immediate', target: 'self', customHandler: 'groundwork_3_powers_check' })
    .register();

  defineEffect('power', 'Dominator')
    .description('Trong Combat: Thua ít nhất 2/3 round đầu, Debuff: đối phương -1 all stats.')
    .weight(0.78)
    .effect({ type: 'debuff', stat: 'all', value: -1, timing: 'during_combat', target: 'opponent', duration: 'combat', customHandler: 'dominator_2_of_3_loses' })
    .register();
  ...
  defineEffect('power', 'Stat Absorption')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'random', value: 1, timing: 'after_combat_win', target: 'self', customHandler: 'stat_absorption_opponent_highest' })
    .register();

  defineEffect('power', 'Golden Vow')
    .description('Trong combat: Khởi đầu với +1 điểm.')
    .weight(0.78)
    .effect({ type: 'combat_points', points: 1, timing: 'before_combat', target: 'self' })
    .register();
  ...
  defineEffect('power', 'Acid Breath')
    ...
    .effect({ type: 'grant_power', grantType: 'power', grantName: 'Poison Breath', timing: 'after_combat_win', target: 'self' })
    .register();
  ...
  defineEffect('power', 'Poison Breath')
    ...
    .weight(0.78)
    .debuffOpponent('iq', 1)
    .effect({ type: 'grant_power', grantType: 'power', grantName: 'Garlic Breath', timing: 'after_combat_lose', target: 'self' })
    .register();
  ...
  defineEffect('power', 'Zoltraak')
    ...
    .weight(0.78)
    .addStat('biq', 3)
    .effect({ type: 'custom', timing: 'during_combat', target: 'self', customHandler: 'zoltraak_double_biq_round' })
    .register();

  defineEffect('power', 'Encroaching Shadow')
    .description('Trong combat: 75% nhận +7 Speed.')
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'speed', value: 7, timing: 'before_combat', target: 'self', duration: 'combat', customHandler: 'encroaching_shadow_wheel' })
    .register();
  ...
  defineEffect('power', 'No Stopping Me')
    ...
    .weight(0.78)
    .effect({ type: 'stat_modifier', stat: 'iq', value: 3, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'no_stopping_me_alternating_check' })
    .effect({ type: 'stat_modifier', stat: 'biq', value: 3, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'no_stopping_me_alternating_check' })
    .effect({ type: 'stat_modifier', stat: 'ma', value: 3, timing: 'during_combat', target: 'self', duration: 'combat', customHandler: 'no_stopping_me_alternating_check' })
    .register();

  defineEffect('power', 'Mystifying Murmur')
    .description('Trong combat: Đối thủ nhận Debuff: -3 Dura.')
    .weight(0.78)
    .effect({ type: 'debuff', stat: 'durability', value: -3, timing: 'during_combat', target: 'opponent' })
    .register();

  defineEffect('power', 'Spell Flux')
    .description('1 Power kích hoạt "Trong Combat" của bạn kích hoạt lần đầu sẽ được kích hoạt 2 lần.')
    .weight(0.78)
    .effect({ type: 'custom', timing: 'during_combat', target: 'self', customHandler: 'spell_flux_double_first_in_combat' })
    .register();
  ...
  defineEffect('power', 'Cinder Flickering')
    .description('Sau Combat: Khi 2 người có Cinder Flickering trong cùng 1 Combat, người thắng nhận Char Dev "Lord of Cinder".')
    .weight(0)
    .effect({ type: 'custom', timing: 'after_combat', target: 'self', customHandler: 'cinder_flickering_check' })
    .register();

  defineEffect('power', 'Darwin Evolution Theory')
    .description('Sau Combat: Thăng hạng chủng tộc của mình lên 1 bậc. (Cả tộc của bạn được nâng bậc)')
    .weight(0)
    .effect({ type: 'custom', timing: 'after_combat', target: 'self', customHandler: 'darwin_evolution_race_up' })
    .register();

  defineEffect('power', 'Algorithms Are Clear')
    .description('Sau Combat: Cộng tổng Base Stat đối thủ chia 6, thay thế Base Stat thấp nhất của bạn thành con số đó.')
    .weight(0)
    .effect({ type: 'custom', timing: 'after_combat', target: 'self', customHandler: 'algorithms_are_clear_replace_lowest' })
    .register();

  defineEffect('power', 'Eternal Mangekyou Sharingan')
    .description('Trước Combat: Kích hoạt ngẫu nhiên 1 trong 3 hiệu ứng - Amaterasu: đối thủ -6 Dura; Tsukuyomi: đối thủ -6 IQ; Susanoo: đối thủ -6 Strength.')
    .weight(0)
    .effect({ type: 'custom', timing: 'before_combat', target: 'opponent', customHandler: 'eternal_mangekyou_random_effect' })
    .register();

  defineEffect('power', 'Adapt')
    .description('Ghi nhớ mọi Power gặp. Trước combat: Vô hiệu hóa toàn bộ Power của đối thủ mà bản thân đã ghi nhớ.')
    .weight(0)
    .effect({ type: 'custom', timing: 'before_combat', target: 'opponent', customHandler: 'adapt_disable_known_powers' })
    .register();
  ...
  defineEffect('power', 'Super Lucky')
    .description('Khi thua trận, 15% lật kèo thắng. Nếu không, thêm 1 cơ hội 10% lật kèo.')
    .weight(0)
    .effect({ type: 'custom', timing: 'after_combat_lose', target: 'self', customHandler: 'super_lucky_comeback_check' })
    .register();

  defineEffect('power', "Valkyrie's Blessing")
    .description("Miễn nhiễm AIDS. Miễn nhiễm Debuff. Tie-break 66% nghiêng về bạn. Power này không thể bị tác động/vô hiệu/xóa bỏ.")
    .weight(0)
    .effect({ type: 'immunity', immuneTo: ['aids', 'debuff'], timing: 'immediate', target: 'self' })
    .effect({ type: 'immunity', immuneTo: ['remove_valkyrie_blessing'], timing: 'immediate', target: 'self' })
    .register();
  ...
  defineEffect('power', "Sovngarde's Blessing")
    .description('Trong combat: Khởi đầu trận đấu với +1 điểm và Buff: +1 All Stats.')
    .weight(0)
    .effect({ type: 'combat_points', points: 1, timing: 'before_combat', target: 'self' })
    .effect({ type: 'stat_modifier', stat: 'all', value: 1, timing: 'before_combat', target: 'self', duration: 'combat' })
    .register();

  defineEffect('power', 'Glory glory Man United')
    .description('Trong combat: Với mỗi bàn thua EPL gần nhất của MU, -1 all stats. MU thắng ≥3-0: +7 all stats.')
    .weight(0)
    .effect({ type: 'custom', timing: 'during_combat', target: 'self', customHandler: 'glory_man_united_epl_check' })
    .register();

  defineEffect('power', 'Cold Mirage')
    .description('Trong combat: Sau mỗi round thắng, quay 1 chỉ số từ các round chưa thi đấu. Đối thủ không nhận điểm khi thắng những round bị quay ra.')
    .weight(0)
    .effect({ type: 'custom', timing: 'on_round_win', target: 'self', customHandler: 'cold_mirage_spin_unused_round' })
    .register();

  defineEffect('power', 'Golden Parry')
    .description('Trong combat: Sau mỗi round thua, đối thủ có 35% không nhận được điểm.')
    .weight(0)
    .effect({ type: 'custom', timing: 'on_round_lose', target: 'self', conditions: [{ type: 'probability', chance: 35 }], customHandler: 'golden_parry_deny_opponent_point' })
    .register();
  ...
  defineEffect('summon', 'Chihuahua').description('-1 all stats.').weight(10).addAllStats(-1).register();
  defineEffect('summon', 'Mufasa').description('+3 Strength.').weight(12).addStat('strength', 3).register();
  defineEffect('summon', 'Pack of Wolves').description('+3 Speed.').weight(12).addStat('speed', 3).register();
  defineEffect('summon', 'Earth Golem').description('+3 Durability.').weight(12).addStat('durability', 3).register();
  defineEffect('summon', 'Water Elemental').description('+3 IQ.').weight(12).addStat('iq', 3).register();
  defineEffect('summon', 'Imp').description('+3 BIQ.').weight(12).addStat('biq', 3).register();
  defineEffect('summon', 'Igris').description('+3 Martial Arts.').weight(12).addStat('ma', 3).register();
  defineEffect('summon', 'Numby')
    .description('Trong Combat: +4 vào 1 chỉ số ngẫu nhiên.')
    .weight(12)
    .effect({ type: 'stat_modifier', stat: 'random', value: 4, timing: 'during_combat', target: 'self' })
    .register();
  defineEffect('summon', "Wyvern's Egg")
    .description('Ở trận chung kết tổng, nhận 2 điểm khởi đầu.')
    .weight(3)
    .effect({ type: 'combat_points', points: 2, timing: 'before_combat', target: 'self', conditions: [{ type: 'bracket', bracket: 'finals' }] })
    .register();
  defineEffect('summon', "Creator's Cat")
    .description("Nhận 1 lần Char Dev: \"Creator's Favor\".")
    .weight(3)
    .effect({ type: 'grant_char_dev', grantType: 'char_dev', grantName: "Creator's Favor", grantCount: 1, timing: 'immediate', target: 'self' })
    .register();
}
