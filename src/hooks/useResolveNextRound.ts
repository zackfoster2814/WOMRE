...
import {
  PvPPlayerData, CombatResult, StepCombatState, CarryOverEffect,
...
import { calcStatsWithDisabled, buildInventoryList } from "../utils/combatStats";
...
export interface UseResolveNextRoundParams {
  ...
  afterCombatSpinResults: Record<string, { label: string; isSuccess: boolean; meta?: Record<string, unknown> }>;
  // Setters
  setAfterCombatEntries: Dispatch<SetStateAction<AfterCombatEntry[]>>;
  setAfterCombatSpinResults: Dispatch<SetStateAction<Record<string, { label: string; isSuccess: boolean; meta?: Record<string, unknown> }>>>;
  ...
  preBiqFiredHandlersRef: MutableRefObject<{ p1: Set<string>; p2: Set<string> } | null>;
  // Functions
  spawnStatBubbles: (bubbles: Array<{ player: "player1" | "player2"; text: string; isPositive: boolean }>) => void;
  getPerRoundEffects: (char: Character | undefined, playerNo?: number) => { onWin: string[]; onLose: string[]; onTie: string[] };
  computeRoundPoints: (side: "player1" | "player2", winner: "player1" | "player2" | "tie", roundIdx: number, effects: { onWin: string[]; onLose: string[]; onTie: string[] }, statKey?: string, roundsWonBefore?: number) => { pts: number; pending: boolean; color: string; autoApplied?: boolean; engineBase?: number };
  EffectRegistry: typeof EffectRegistry;
  EffectResolver: typeof EffectResolver;
  computeRoundStep: (roundIndex: number, state: StepCombatState, p1: PvPPlayerData, p2: PvPPlayerData, otpStats?: Record<string, string>, hmStats?: Record<string, string>, forcedWinner?: "player1" | "player2" | null) => { newState: StepCombatState; log: any };
...
export function useResolveNextRound(params: UseResolveNextRoundParams) {
  const resolveNextRound = () => {
    const {
      ...
      roundtableSubMode, zoltraakBiq2Pending,
      ...
      pendingAfterCombatBuildRef, pendingCrueltyAfterCombatRef,
      pendingFinalizeStateRef, preBiqFiredHandlersRef,
  ...
  if (nextIdx === 6) {
    ...
    if (pendingMA) {
      ...
      pendingCrueltyAfterCombatRef.current = (w: "player1" | "player2", s1?: number, s2?: number) =>
