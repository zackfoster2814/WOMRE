...
import { readDriveFile, writeDriveFile, isDriveConfigured } from "../utils/googleDrive";
...
const RACE_TIERS: Record<string, number> = {
  God: 1, Dragon: 2, Primordial_Being: 3, Demon: 4, Angel: 5,
  Dryad: 6, Elf: 7, Troll: 8, Dwarf: 9, Orc: 10,
  Werebeast: 11, Gnome: 12, Uma: 13, Fairy: 14, Merfolk: 15,
  Undead: 16, Skeleton: 17, Vampire: 18, Reincarnator: 19,
  Human: 20, Halfling: 21, Goblin: 22,
};
...
const CreateTab = ({ players, roundData, setRoundData, saveToDrive }: CreateTabProps) => {
  ...
  const handleSpinComplete = useCallback(async (item: WheelItem) => {
    if (!roundData) return;
    ...
    if (isPlayer1) {
      ...
      newMatches.push({
        ...
        player1: { no: drawnPlayer.id, name: drawnPlayer.name, username: drawnPlayer.username },
        player2: null,
        winner: null,
        score: null,
        specialEvent: null,
      ...
      if (newMatches[matchIdx]) {
        newMatches[matchIdx] = {
          ...newMatches[matchIdx],
          player2: { no: drawnPlayer.id, name: drawnPlayer.name, username: drawnPlayer.username },
        };
      }
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {remainingPlayers.length > 0 ? (
          <div className="max-w-md mx-auto">
            <WheelCanvas
              ...
              onSpin={() => {
                if (remainingPlayers.length > 0 && !isSpinning) setIsSpinning(true);
              }}
            />
          </div>
        ) : (
          <div className="text-center py-8">
            <p className="text-green-400 text-xl">All {roundData?.drawOrder.length || 0} players drawn!</p>
          </div>
        )}
        ...
        {lastDrawnPlayer && (
          <div className="mt-4 p-3 bg-yellow-600/20 border border-yellow-600 rounded-lg">
            <p className="text-yellow-400 text-center">
              Last Drawn: <span className="font-bold">No.{lastDrawnPlayer.id} {lastDrawnPlayer.name}</span>
            </p>
          </div>
        )}
      ...
      <div className="bg-gray-800/50 rounded-lg p-4">
        ...
        <div className="max-h-[600px] overflow-y-auto space-y-2">
          {roundData?.matches.slice().reverse().map((match) => (
            <div
            ...
            >
              <div className="text-xs text-gray-400 mb-1">Match #{match.matchNumber}</div>
...
const BracketTab = ({ players, roundData, setRoundData, saveToDrive }: BracketTabProps) => {
  ...
  const handleMatchUpdate = useCallback(async (updatedMatch: MatchData) => {
    if (!roundData) return;
  ...
  if (!roundData || roundData.matches.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-400 text-xl">No matches yet. Go to Create tab to draw players.</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex gap-4 text-sm">
          ...
          <span className="text-green-400">Completed: <span className="font-bold">{stats.completed}</span></span>
          <span className="text-yellow-400">Pending: <span className="font-bold">{stats.pending}</span></span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 max-h-[70vh] overflow-y-auto">
        {filteredMatches.map((match) => (
          ...
          >
            ...
            {match.specialEvent && (
              <div className="mt-1 text-center text-xs text-orange-400 truncate">{match.specialEvent}</div>
            )}
          </button>
        ))}
      </div>
...
const BattleModal = ({ match, players, onClose, onSave }: BattleModalProps) => {
  ...
  const player1 = useMemo(() => players.find((p) => p.id === match.player1?.no), [players, match]);
  const player2 = useMemo(() => players.find((p) => p.id === match.player2?.no), [players, match]);

  // Calculate stats
  const p1Stats = useMemo(() => {
    if (!player1) return null;
    const effects = EffectResolver.calculateCharacterEffects(player1.character, { isPvE: false });
    return {
  ...
  const p2Stats = useMemo(() => {
    ...
    const effects = EffectResolver.calculateCharacterEffects(player2.character, { isPvE: false });
    return {
      str: effects.totalStats.strength,
      spd: effects.totalStats.speed,
      dur: effects.totalStats.durability,
  ...
  const runBattleInstant = () => {
    ...
    for (const { key, label } of STAT_ORDER) {
      ...
      if (v1 > v2) { winner = "p1"; s1++; }
      else if (v2 > v1) { winner = "p2"; s2++; }
      else { winner = "tie"; }
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={(e) => {
      ...
      <div ref={modalRef} className="bg-gray-900 rounded-2xl border border-gray-700 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="p-6">
          <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-start">
            ...
            <div className="flex flex-col items-center justify-center pt-8">
              ...
              {tieBreaker && (
                <div className="text-xs text-yellow-400 text-center">{tieBreaker}</div>
              )}
              {overallWinner && battleDone && (
                <div className={`text-sm font-bold mt-1 ${
                  overallWinner === "p1" ? "text-blue-400" : "text-red-400"
                }`}>
                  {overallWinner === "p1" ? match.player1?.name : match.player2?.name} WINS!
                </div>
              )}
            </div>
          ...
          {rounds.length > 0 && (
            <div className="mt-6 space-y-2">
              {rounds.map((round, idx) => (
                ...
                >
                  <div className={`text-right text-lg font-bold px-3 py-2 rounded ${
                  ...
                  <div className="text-center text-sm font-bold text-gray-300">{round.label}</div>
                  <div className={`text-left text-lg font-bold px-3 py-2 rounded ${
          ...
          <div className="mt-6 grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1">Special Event</label>
              <input
                type="text"
                value={specialEvent}
            ...
            {!battleDone && (
              <>
                <button
                  ...
                  className="px-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-bold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  Battle!
                </button>
                <button
                  onClick={() => setManualWinner("p1")}
                  className="px-4 py-3 bg-blue-700 hover:bg-blue-800 text-white rounded-lg text-sm"
                >
                  {match.player1?.name} Wins (Manual)
                </button>
                <button
                  onClick={() => setManualWinner("p2")}
                  className="px-4 py-3 bg-red-700 hover:bg-red-800 text-white rounded-lg text-sm"
                >
...
const PlayerCard = ({ player, playerRef, stats, side, isWinner }: PlayerCardProps) => {
  ...
  return (
    <div className={`p-4 rounded-lg border-2 transition-all ${
      ...
      <h3 className="text-lg font-bold text-white truncate">{playerRef?.name || "TBD"}</h3>
      ...
      {stats && (
        <div className="mt-3 space-y-1">
          {STAT_ORDER.map(({ key, label }) => (
            <div key={key} className={`flex items-center gap-2 text-sm ${side === "right" ? "flex-row-reverse" : ""}`}>
              ...
              <span className="text-white font-mono w-6 text-center">{stats[key]}</span>
            </div>
          ))}
        </div>
      )}
